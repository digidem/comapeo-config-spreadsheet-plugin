...
function extractTranslations(translationsData: any): NormalizedTranslations {
  ...
  for (const language in translationsData) {
    ...
    for (const key in languageData) {
      try {
        ...
        if (typeof translationData.message === "string") {
          ...
          typeof translationData.message === "object"
        ) {
    ...
    console.log(
      `Extracted ${normalizedTranslations[language].length} translations for ${language}`,
    );
  }
...
function parseTranslationKey(key: string): {
  type: "preset" | "field" | "option" | "helperText" | "other";
...
function applyPresetTranslations(
  ...
  const categoryRows = presets.map((preset) => {
    ...
    langCodes.forEach((langCode) => {
      ...
      const presetTranslations = translations.filter(
        (t) => t.type === "preset" && t.normalizedKey === preset.icon,
      );

      // Use the first matching translation or empty string
      const translation =
        presetTranslations.length > 0 ? presetTranslations[0].message : "";
...
function applyFieldLabelTranslations(
  ...
  const labelRows = fields.map((field) => {
    ...
    langCodes.forEach((langCode) => {
      const translations = normalizedTranslations[langCode];
      const fieldTranslations = translations.filter(
        (t) => t.type === "field" && t.normalizedKey === field.tagKey,
      );

      // Use the first matching translation or empty string
      const translation =
        fieldTranslations.length > 0 ? fieldTranslations[0].message : "";
      row.push(translation);
    });
...
function applyHelperTextTranslations(
  ...
  const helperRows = fields
    ...
    .map((field) => {
      ...
      langCodes.forEach((langCode) => {
        const translations = normalizedTranslations[langCode];
        const helperTranslations = translations.filter(
          (t) => t.type === "helperText" && t.normalizedKey === field.tagKey,
        );

        // Use the first matching translation or empty string
        const translation =
          helperTranslations.length > 0 ? helperTranslations[0].message : "";
        row.push(translation);
      });
