...
function testFieldExtraction(url?: string) {
  ...
  try {
    ...
    const dialog = ui.showModelessDialog(htmlOutput, 'Testing Field Extraction');
    ...
    const tempFolder = DriveApp.createFolder('Field_Extraction_Test_' + new Date().getTime());
    ...
    try {
      ...
      log(`- Fields: ${configData.fields ? configData.fields.length : 0}`, 'info');
      ...
      if (configData.fields && configData.fields.length > 0) {
        ...
        sampleFields.forEach((field, index) => {
          log(`Sample field ${index + 1}: ${field.label || field.id || 'unnamed'} (${field.type || 'unknown type'})`, 'info');
        });
      } else {
    ...
    try {
      ...
      // Apply fields
      if (configData.fields && configData.fields.length > 0) {
        log(`Applying ${configData.fields.length} fields to Details sheet`, 'info');
        ...
        if (newDetailsSheet) {
          ...
          log(`Details sheet now has ${lastRow - 1} fields (header row excluded)`, 'success');
          
          if (lastRow <= 1) {
            log('No fields were added to the Details sheet!', 'error');
          } else if (lastRow - 1 !== configData.fields.length) {
            log(`Warning: Expected ${configData.fields.length} fields but found ${lastRow - 1}`, 'warning');
          }
        } else {
