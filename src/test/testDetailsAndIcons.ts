...
function testDetailsAndIcons() {
  ...
  try {
    ...
    const dialog = ui.showModelessDialog(htmlOutput, 'Testing Details and Icons Extraction');
    ...
    const url = 'https://github.com/digidem/mapeo-config-es/releases/download/v1.0.0/mapeo-config-es-v1.0.0.mapeosettings';
    ...
    const tempFolder = DriveApp.createFolder('Details_Icons_Test_' + new Date().getTime());
    ...
    try {
      ...
      log(`Extraction successful: ${extractionResult.files.length} files extracted`, 'success');
    } catch (error) {
    ...
    try {
      ...
      log(`- Metadata: ${configData.metadata ? 'Present' : 'Not present'}`, 'info');
      ...
      try {
        ...
        if (newCategoriesSheet) {
          const categoriesCount = Math.max(0, newCategoriesSheet.getLastRow() - 1); // Subtract header row
          log(`Categories sheet has ${categoriesCount} categories`, 'info');
          
          // Check if icons are present
          let iconsCount = 0;
          if (categoriesCount > 0) {
            const iconRange = newCategoriesSheet.getRange(2, 2, categoriesCount, 1);
            const iconValues = iconRange.getValues();
            iconsCount = iconValues.filter(row => row[0] && row[0].toString().trim() !== '').length;
            log(`Categories sheet has ${iconsCount} icons (${Math.round(iconsCount/categoriesCount*100)}% coverage)`, 
        ...
        if (newDetailsSheet) {
          ...
          log(`Details sheet has ${detailsCount} fields`, 
          ...
          if (detailsCount === 0) {
            ...
            log(`Warning: Expected ${configData.fields.length} fields but found ${detailsCount}`, 'warning');
          }
        } else {
        ...
        if (backupSheets['Categories'] && categoriesSheet) {
          ...
          categoriesSheet.getRange(1, 1, backupSheets['Categories'].length, backupSheets['Categories'][0].length)
        ...
        if (backupSheets['Details'] && detailsSheet) {
          detailsSheet.clear();
          detailsSheet.getRange(1, 1, backupSheets['Details'].length, backupSheets['Details'][0].length)
