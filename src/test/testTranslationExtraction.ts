...
function testTranslationExtraction(url?: string) {
  ...
  try {
    ...
    const dialog = ui.showModelessDialog(
      htmlOutput,
      "Testing Translation Extraction",
    );
    ...
    const tempFolder = DriveApp.createFolder(
      "Translation_Extraction_Test_" + new Date().getTime(),
    );
    ...
    try {
      ...
      if (url.endsWith(".comapeocat") || url.endsWith(".zip")) {
        ...
        const translationsFile = extractedFiles.find(
          (file) => file.getName() === "translations.json",
        );
        if (translationsFile) {
        ...
        log(
          "Detected .mapeosettings or .tar file, using tar extractor",
          "info",
        );
        try {
          ...
          log(
            `Tar extraction failed, trying to parse as JSON: ${tarError}`,
            "warning",
          );
    ...
    try {
      ...
      for (const language in normalizedTranslations) {
        ...
        log(
          `Language: ${language} - ${translations.length} translations`,
          "info",
        );

        // Count by type
        const presetCount = translations.filter(
          (t) => t.type === "preset",
        ).length;
        const fieldCount = translations.filter(
          (t) => t.type === "field",
        ).length;
        const helperTextCount = translations.filter(
          (t) => t.type === "helperText",
        ).length;
        const optionCount = translations.filter(
          (t) => t.type === "option",
        ).length;
        const otherCount = translations.filter(
          (t) => t.type === "other",
        ).length;
        ...
        if (translations.length > 0) {
          log("Examples:", "info");
          const examples = translations.slice(
            0,
            Math.min(5, translations.length),
          );
          examples.forEach((t) => {
