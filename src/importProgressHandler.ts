 ...
 */
function createProgressHandler(callback: Function): (data: ProgressData) => void {
  ...
  try {
    ...
    applyConfigurationToSpreadsheet(configData, {
      onProgress: (stage, percent) => {
        ...
        const mappedPercent = 75 + Math.round(percent * 0.20);
        progressHandler({
          percent: mappedPercent,
          stage: 'Updating spreadsheet',
          detail: stage
    ...
    try {
      ...
      try {
        ...
        progressHandler({
          percent: 0,
          stage: 'Error',
          detail: 'Could not parse the Mapeo settings file. The file may be corrupted.'
        });
