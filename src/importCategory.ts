...
function showImportCategoryDialog() {
  ...
  const message = importCategoryDialogTexts[locale].message.map(msg => `<p>${msg}</p>`).join("\n");
    ...
    <form id="uploadForm">
      <div class="file-upload-container">
        <label for="file" class="file-upload-label">
          ...
          <span class="upload-text">${importCategoryDialogTexts[locale].buttonText}</span>
        </label>
  ...
  const fullMessage = message + uploadForm;
  const html = generateDialog(title, fullMessage);
  ...
  if (sheet) {
    ...
    const fieldRows = fields.map(field => {
      ...
      if (field.options && field.options.length > 0) {
        optionsStr = field.options.map(opt => opt.label).join(', ');
      }
