...
function processImportedCategoryFile(
  ...
  try {
        ...
        extractionResult.validationErrors.forEach((errorMsg) => {
          const error = createImportError(
            ...
            `Validation error: ${errorMsg}`,
            {
              recoverable: false,
              suggestedAction:
                "Check that the file contains all required components.",
            },
          );
          addImportError(error);
        });
    ...
    // Show error dialog
    showErrorDialog("Import Failed", result.errors || [importError]);
...
function extractConfigurationData(
  unzippedFiles: GoogleAppsScript.Base.Blob[],
  ...
  unzippedFiles.forEach((file, index) => {
    ...
    reportProgress(
      `Processing file ${index + 1}/${unzippedFiles.length}`,
      progress,
    );

    console.log(
      `Processing file: ${fileName} (${file.getContentType()}, size: ${file.getBytes().length} bytes)`,
    );
    ...
    try {
      if (fileName.endsWith(".json")) {
        ...
        console.log(
          `Parsed JSON file: ${fileName}, keys: [${Object.keys(jsonContent).join(", ")}]`,
        );
      } else if (
        ...
        iconFiles.push(file);
        console.log(`Found icon file: ${fileName} (${file.getContentType()})`);
      } else {
        console.log(`Skipping file: ${fileName} (not JSON or icon)`);
      }
    } catch (error) {
  ...
  console.log(
    `Found ${Object.keys(jsonFiles).length} JSON files and ${iconFiles.length} icon files`,
  );
  ...
  if (
    ...
    // Process fields
    const fieldFiles = Object.keys(jsonFiles).filter((f) =>
      f.startsWith("fields/"),
    );
    ...
    fieldFiles.forEach((f) => {
      ...
      if (field) {
        ...
        console.log(
          `Added field from ${f}: ${field.label || field.id || "unnamed field"}`,
        );
      }
    });
  ...
  // Check for CoMapeo flat structure (presets.json, fields.json, etc.)
    ...
    jsonFiles["metadata.json"] &&
    (jsonFiles["presets.json"] ||
      jsonFiles["fields.json"] ||
      jsonFiles["translations.json"])
  ) {
    ...
    if (jsonFiles["presets.json"]) {
      if (jsonFiles["presets.json"].presets) {
        if (
          typeof jsonFiles["presets.json"].presets === "object" &&
          !Array.isArray(jsonFiles["presets.json"].presets)
        ) {
          // Convert object to array
          combinedConfig.presets = Object.entries(
            jsonFiles["presets.json"].presets,
    ...
    if (jsonFiles["fields.json"]) {
      if (jsonFiles["fields.json"].fields) {
        if (
          typeof jsonFiles["fields.json"].fields === "object" &&
          !Array.isArray(jsonFiles["fields.json"].fields)
        ) {
          // Convert object to array
          combinedConfig.fields = Object.entries(
            jsonFiles["fields.json"].fields,
          ).map(([id, field]) => {
            ...
            console.log(
              `Extracted field from fields.json: ${normalizedField.label || id}`,
            );
            return normalizedField;
          });
          console.log(
            `Converted ${combinedConfig.fields.length} fields from object to array format`,
          );
        } else {
          // Process array format
          combinedConfig.fields = jsonFiles["fields.json"].fields.map(
            (field) => {
              // Ensure field has required properties
              ...
              console.log(
                `Extracted field from fields.json array: ${field.label || field.id || "unnamed field"}`,
              );
              return field;
            },
          );
          console.log(
            `Found ${combinedConfig.fields.length} fields in array format`,
          );
        }
      } else {
        combinedConfig.fields = [];
      ...
      if (
        typeof jsonFiles["presets.json"].fields === "object" &&
        !Array.isArray(jsonFiles["presets.json"].fields)
      ) {
        // Convert object to array
        combinedConfig.fields = Object.entries(
          jsonFiles["presets.json"].fields,
        ).map(([id, field]) => {
          const normalizedField = {
            id,
            tagKey: id,
          ...
          console.log(
            `Extracted field from presets.json: ${normalizedField.label || id}`,
          );
          return normalizedField;
        });
        console.log(
          `Extracted ${combinedConfig.fields.length} fields from presets.json`,
        );
      } else {
        // Process array format
        combinedConfig.fields = jsonFiles["presets.json"].fields.map(
          (field) => {
            // Ensure field has required properties
            if (!field.id && !field.tagKey && field.label) {
              field.id = slugify(field.label);
            ...
            console.log(
              `Extracted field from presets.json array: ${field.label || field.id || "unnamed field"}`,
            );
            return field;
          },
        );
        console.log(
          `Found ${combinedConfig.fields.length} fields in presets.json`,
        );
      }
    } else {
      combinedConfig.fields = [];
    ...
    combinedConfig.fields = combinedConfig.fields.filter((field) => {
      ...
      if (!field.label) {
        console.warn(
          `Field missing label: ${field.id || field.tagKey || "unknown"}`,
        );
  ...
  let iconsFolder;
  try {
    iconsFolder = tempFolder.createFolder("processed_icons");
    console.log(
      `Created icons folder: ${iconsFolder.getName()} (${iconsFolder.getId()})`,
    );
  } catch (error) {
    console.error(`Error creating icons folder: ${error}`);
    iconsFolder = tempFolder;
  }

  // Map to track processed icons
  ...
  iconFiles.forEach((file, index) => {
    try {
      const fileName = file.getName();
      const progress = 70 + Math.round((index / iconFiles.length) * 20); // 70-90%
      reportProgress(
        `Processing icon ${index + 1}/${iconFiles.length}`,
        progress,
      );
      ...
      console.log(
        `Processing icon file ${index + 1}/${iconFiles.length}: ${fileName} -> ${iconName} (${file.getContentType()})`,
      );
      ...
      console.log(`Processed icon file: ${fileName} -> ${iconName}`);
      console.log(`  - URL: ${iconUrl}`);
      console.log(`  - ID: ${iconObj.id}`);
      console.log(
        `  - Added to icons array at index ${combinedConfig.icons.length - 1}`,
      );
    } catch (error) {
      console.error(`Error processing icon file at index ${index}:`, error);
    }
  });

  console.log(`Processed ${combinedConfig.icons.length} icons successfully`);
  console.log(
    `Icon names: [${combinedConfig.icons.map((icon) => icon.name).join(", ")}]`,
  );
  ...
  if (combinedConfig.presets.length > 0 && combinedConfig.icons.length === 0) {
    console.log(
      "No icons found but presets exist. Generating default icons...",
    );
    ...
    combinedConfig.presets.forEach((preset) => {
      if (preset.icon && !iconNameMap.has(preset.icon)) {
        try {
          ...
          console.log(
            `Generated default icon for preset: ${preset.name} (${preset.icon})`,
          );
        } catch (error) {
          console.warn(
            `Error generating default icon for preset ${preset.name}:`,
            error,
          );
        }
      }
  ...
  if (!configData.packageJson && configData.metadata) {
    configData.packageJson = {
      ...
      scripts: {
        build:
          "mkdir -p build && mapeo-settings build -l 'en' -o build/${configData.metadata.dataset_id}-v${configData.metadata.version}.comapeocat",
  ...
  if (Array.isArray(configData.fields)) {
    ...
    configData.fields.forEach((field: any) => {
      if (field) {
        ...
        console.log(
          `Extracted field: ${normalizedField.label} (${normalizedField.tagKey})`,
        );
      }
    });
...
function applyConfigurationToSpreadsheet(
  ...
  console.log("Starting spreadsheet update with configuration data");
  ...
  - Messages: ${configData.messages ? Object.keys(configData.messages).length : 0} languages`);

  reportProgress("Initializing spreadsheet update", 0);
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  console.log(
    `Active spreadsheet: ${spreadsheet.getName()} (${spreadsheet.getId()})`,
  );
  ...
  console.log("Prepared all required sheets");

  // Apply metadata
  reportProgress("Updating metadata", 20);
  if (configData.metadata) {
    console.log(
      `Applying metadata with ${Object.keys(configData.metadata).length} keys`,
    );
    applyMetadata(spreadsheet, configData.metadata);
    console.log("Metadata applied successfully");
  } else {
    console.warn("No metadata to apply");
  }
  ...
  if (configData.presets && configData.presets.length > 0) {
    console.log(
      `Applying ${configData.presets.length} categories with ${configData.icons ? configData.icons.length : 0} icons`,
    );
    ...
    console.warn("No categories to add");
  ...
  if (configData.fields && configData.fields.length > 0) {
    ...
    if (detailsSheet) {
      ...
      console.log(
        `Details tab now has ${lastRow - 1} fields (header row excluded)`,
      );
      if (lastRow <= 1) {
        console.error("No fields were actually written to the Details sheet!");
      }
    } else {
      console.error("Details tab not found after applying fields");
    }
  ...
  if (configData.messages && Object.keys(configData.messages).length > 0) {
    console.log(
      `Applying translations for ${Object.keys(configData.messages).length} languages`,
    );
    applyTranslations(
      spreadsheet,
      configData.messages,
    ...
    reportProgress(
      `Added translations for ${Object.keys(configData.messages).length} languages`,
      95,
    );
    console.log("Translations applied successfully");
  } else {
    console.warn("No translations to add");
...
function applyCategories(
  ...
  console.log(
    `Applying ${presets.length} categories to Categories sheet with ${icons.length} icons`,
  );
  console.log(`Icon objects: ${JSON.stringify(icons.slice(0, 3))}`); // Log first 3 icons for debugging
  ...
  icons.forEach((icon) => {
    if (icon.name && icon.svg) {
      ...
      console.log(
        `Mapped icon: ${icon.name} -> ${icon.svg.substring(0, 30)}...`,
      );
    } else {
      console.warn(
        `Skipping icon with missing name or svg: ${JSON.stringify(icon)}`,
      );
    }
  });

  console.log(`Created icon map with ${Object.keys(iconMap).length} entries`);
  console.log(
    `Icon map keys: [${Object.keys(iconMap).slice(0, 10).join(", ")}${Object.keys(iconMap).length > 10 ? "..." : ""}]`,
  );

  // Prepare category rows
  const categoryRows = presets.map((preset) => {
    console.log(`Processing preset: ${JSON.stringify(preset)}`);
    ...
    if (preset.icon && iconMap[preset.icon]) {
      ...
      console.log(
        `Found icon for ${preset.name} by direct match: ${preset.icon} -> ${iconUrl.substring(0, 30)}...`,
      );
    }
    // Then try to find by icon ID
    else {
      console.log(
        `No direct match for icon '${preset.icon}', trying to find by object search...`,
      );
      ...
      if (iconObj) {
        ...
        console.log(
          `Found icon for ${preset.name} by object search: ${iconObj.name} -> ${iconUrl.substring(0, 30)}...`,
        );
      } else {
        console.warn(
          `No icon found for ${preset.name} (icon key: ${preset.icon})`,
        );
      }
    }
    ...
    console.log(
      `Adding category: ${preset.name}${iconUrl ? " with icon" : " WITHOUT ICON"}${fields ? " with fields: " + fields : " WITHOUT FIELDS"}`,
    );
  ...
  if (categoryRows.length > 0) {
    ...
    const writtenData = sheet
      .getRange(2, 1, categoryRows.length, 3)
      .getValues();
    console.log(`First row written: ${JSON.stringify(writtenData[0])}`);
    console.log(
      `Icons column has values: ${writtenData.some((row) => row[1] !== "")}`,
    );
    console.log(
      `Details column has values: ${writtenData.some((row) => row[2] !== "")}`,
    );
  } else {
...
function applyFields(
  ...
  console.log(`Field objects sample: ${JSON.stringify(fields.slice(0, 3))}`); // Log first 3 fields
  ...
  sheet
    .getRange(1, 1, 1, 6)
    .setValues([
      ["Label", "Helper Text", "Type", "Options", "Required", "Universal"],
    ]);
  sheet.getRange(1, 1, 1, 6).setFontWeight("bold");
  console.log("Set headers for Details sheet");

  // Prepare field rows
  const fieldRows = fields.map((field, idx) => {
    console.log(
      `Processing field ${idx + 1}/${fields.length}: ${field.label || field.id || "unnamed"}`,
    );
    ...
    if (field.options && field.options.length > 0) {
      optionsStr = field.options
        ...
        .join(", ");
      console.log(`Field has ${field.options.length} options: ${optionsStr}`);
    }
    ...
    console.log(
      `Adding field: ${field.label} (${typeStr})${optionsStr ? " with options" : ""}, required: ${required}, universal: ${universal}`,
    );
    return [
      field.label,
      field.helperText || "",
  ...
  if (fieldRows.length > 0) {
    console.log(`Writing ${fieldRows.length} field rows to Details sheet`);
    try {
      sheet.getRange(2, 1, fieldRows.length, 6).setValues(fieldRows);
      console.log(`Added ${fieldRows.length} fields to Details sheet`);

      // Verify data was written
      const writtenData = sheet.getRange(2, 1, fieldRows.length, 6).getValues();
      console.log(`First row written: ${JSON.stringify(writtenData[0])}`);
      console.log(
        `Last row written: ${JSON.stringify(writtenData[writtenData.length - 1])}`,
      );
    } catch (error) {
      console.error(`Error writing fields to sheet: ${error}`);
    }
 ...
 * @param fields - Array of field objects
 */
function applyTranslationsFallback(
  spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet,
  messages: any,
  presets: any[],
  fields: any[],
  ...
  if (categorySheet) {
    ...
    const categoryRows = presets.map((preset) => {
      ...
      langCodes.forEach((langCode) => {
        ...
        const translation = langMessages[translationKey]
          ? langMessages[translationKey].message
          : "";
    ...
    if (categoryRows.length > 0) {
      categorySheet
        .getRange(2, 1, categoryRows.length, headers.length)
  ...
  if (labelSheet) {
    ...
    if (labelRows.length > 0) {
      labelSheet
        .getRange(2, 1, labelRows.length, headers.length)
        .setValues(labelRows);
    }
  }

  // Apply helper text translations
  const helperSheet = spreadsheet.getSheetByName(
    "Detail Helper Text Translations",
  );
  if (helperSheet) {
    // Set headers
    ...
    const helperRows = fields
      .filter((field) => field.helperText)
      .map((field) => {
    ...
    if (helperRows.length > 0) {
      helperSheet
        .getRange(2, 1, helperRows.length, headers.length)
        .setValues(helperRows);
    }
  }
  ...
  if (optionSheet) {
    ...
    const optionRows = allOptions.map((option) => {
      ...
      langCodes.forEach((langCode) => {
        ...
        const translationKey =
          "fields." + option.fieldTagKey + ".options." + option.value;
        const translation = langMessages[translationKey]
          ? langMessages[translationKey].message
          : "";
        row.push(translation);
    ...
    if (optionRows.length > 0) {
      optionSheet
        .getRange(2, 1, optionRows.length, headers.length)
