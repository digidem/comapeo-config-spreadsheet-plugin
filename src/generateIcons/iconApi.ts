...
/**
 * Result of an icon API operation
 */
interface IconApiResult<T> {
  ...
  error?: {
    /** Type of error */
    type: "network" | "api" | "timeout" | "parse" | "not-found" | "rate-limit";
...
/**
 * Fetch search data with structured error reporting
 * @param param - Search parameter
 * @returns IconApiResult with data or error details
 */
function fetchSearchDataStructured(param: string): IconApiResult<any[]> {
  // Try to get from cache first
  const cache = CacheService.getScriptCache();
  const cacheKey = `${ICON_SEARCH_CACHE_PREFIX}${param}`;
  const cachedData = cache.get(cacheKey);
    ...
    try {
      ...
      if (responseCode === 404) {
        return {
          ...
          error: {
            type: "not-found",
            message: `No icons found for search term "${param}"`,
            statusCode: responseCode,
            context: { searchUrl, param }
          }
        };
      }
      ...
      if (responseCode >= 400 && responseCode < 500) {
        return {
          ...
          error: {
            type: "api",
            message: `API client error (${responseCode})`,
            statusCode: responseCode,
            context: { searchUrl, param }
          }
        };
        ...
        if (attempt === maxRetries) {
          return {
            ...
            error: {
              ...
              context: { searchUrl, param, attempts: maxRetries + 1 }
            }
          };
        }
      }
  ...
  return {
    ...
    error: {
      type: "network",
      message: `Failed to fetch icon data after ${maxRetries + 1} attempts: ${lastError?.message}`,
      context: { searchUrl, param, attempts: maxRetries + 1, lastError: lastError?.message }
    }
  };
}

/**
 * Generate icon data with structured error reporting
 * @param pngUrl - PNG URL to process
 * @param color - Color hex code
 * @returns IconApiResult with data or error details
 */
function getGenerateDataStructured(pngUrl: string, color: string): IconApiResult<any[]> {
  // Try to get from cache first
  const cache = CacheService.getScriptCache();
  const colorHex = color.split("#")[1];
  const cacheKey = `${ICON_GENERATE_CACHE_PREFIX}${pngUrl}_${colorHex}`;
    ...
    try {
      ...
      if (responseCode >= 200 && responseCode < 300) {
        ...
        return {
          ...
          error: {
            type: "parse",
            message: "API returned invalid data format (missing SVG content)",
            statusCode: responseCode,
            context: { generateUrl, pngUrl, color: colorHex }
          }
        };
      } else {
        // Log non-success status codes for retry
        ...
        if (responseCode === 404) {
          return {
            ...
            error: {
              type: "not-found",
              message: "Icon source image not found",
              statusCode: responseCode,
              context: { generateUrl, pngUrl, color: colorHex }
            }
          };
        ...
        if (attempt === maxRetries) {
          return {
            ...
            error: {
              type: "timeout",
              message: `Request timed out after ${maxRetries + 1} attempts`,
              context: { generateUrl, pngUrl, color: colorHex, attempts: maxRetries + 1 }
            }
          };
        }
      }
  ...
  return {
    ...
    error: {
      type: "network",
      message: `Failed to generate icon after ${maxRetries + 1} attempts: ${lastError?.message}`,
      context: { generateUrl, pngUrl, color: colorHex, attempts: maxRetries + 1, lastError: lastError?.message }
    }
  };
}
