 ...
 */
function sendDataToApiAndGetZip(zipFile: GoogleAppsScript.Base.Blob, metadata: { name: string, version: string }, maxRetries: number = 3): string {
    ...
    try {
      ...
      if (retryCount > 0) {
        ...
        ui.alert(
          ...
          "Attempt " + (retryCount) + " of " + maxRetries + ". Previous attempt failed: " +
          (lastError ? lastError.message : "Unknown error"),
          ui.ButtonSet.OK
        );
      }

      console.log('Posting zip to API URL (attempt ' + (retryCount + 1) + '):', apiUrl);
      const form = {
        file: zipFile
      };
      ...
      if (responseCode === 200) {
        ...
        if (blobSize < minValidFileSize) {
          lastError = new Error("API returned a file that is too small (" + blobSize + " bytes). This is likely an error response.");
        ...
        try {
          ...
          if (!isValidZip) {
            lastError = new Error("API returned an invalid zip file. The file does not have a valid zip signature.");
            console.error(lastError.message);
            retryCount++;
          ...
          lastError = new Error("Failed to save the file: " + saveError.message);
          console.error(lastError.message);
          retryCount++;
          // Wait before retrying
          Utilities.sleep(1000 * Math.pow(2, retryCount));
  ...
  throw new Error("Failed to generate CoMapeo category after " + maxRetries + " attempts: " +
                 (lastError ? lastError.message : "Unknown error"));
}
